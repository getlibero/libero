...
[GoatCounter](https://www.goatcounter.com), in `src/components/Analytics.astro`, on both surfaces
— `Marketing.astro` for the root and `overrides/Head.astro` for the docs. No cookies, no
cross-site identifier, no personal data, nothing to consent to. A project whose pitch is that
credentials stay in one process should not be the one shipping a tracker.

The collection endpoint is `stats.getlibero.com`, a CNAME onto GoatCounter. **`count.js` is
vendored into `public/`**, so a page makes no third-party request at all. The custom domain cannot
serve the script — `stats.getlibero.com/count.js` is a 404 and GoatCounter's own settings page
calls the custom domain "just intended as a convenient vanity domain" — so the choice was our
origin or their CDN, not which of theirs. Ours also takes an unpinned third-party script off every
page, which mattered more than the request did.

The copy is `count.v5.js`, a frozen release rather than the rolling `count.js`: one commit
upstream on 2025-06-09 and untouched since. Pinning to a version that cannot change is what gives
a fixed URL to diff against later; `count.js` moves a few times a year. v3, v4 and v5 are served
today, v6 is not.

`site/vendor.json` records the provenance and the hash. **Changing a vendored file means updating
its hash there in the same commit** — `.github/workflows/vendor-check.yml` fails if they disagree,
because a file whose whole contract is being byte-identical to upstream having been edited is a
worse problem than upstream having moved.
...
The tag is emitted on production builds only, so a dev page fires no beacon. `count.js` separately
refuses to count `localhost`, `127.*` and the private ranges, which is what makes `pnpm preview`
safe: it exercises the real tag without polluting the numbers.

`/discord` is **not** counted, and cannot easily be. It is a meta-refresh stub generated by
Astro's `redirects` config rather than rendered through a layout, and a beacon fired from a page
that navigates immediately is not a beacon that arrives. Discord's own invite page reports how
many times the invite was used, which is the number that question actually wants.

The dashboard is public at [stats.getlibero.com](https://stats.getlibero.com), and the footer
links it. That link is only honest while it stays public — if it is ever made private, remove the
footer entry rather than leaving a disclosure that lands on a login form.
