...
Two hosts, and not by preference. The collection endpoint is `stats.getlibero.com`, a CNAME onto
GoatCounter; the script is `gc.zgo.at/count.js`, because **the custom domain serves `/count` but
not `/count.js`** — that path 404s there. If GoatCounter ever serves the script from custom
domains, moving it removes the third-party request entirely and nothing else changes.

The tag is emitted on production builds only, so a dev page makes no request to the CDN at all.
`count.js` separately refuses to count `localhost`, `127.*` and the private ranges, which is what
makes `pnpm preview` safe: it exercises the real tag without polluting the numbers.
