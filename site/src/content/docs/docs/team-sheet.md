...
It defaults off rather than inheriting your allowlist, and that is the reason the field exists at
all: without it, every channel that already lists tools would have gained an unattended caller for
them the day this shipped, with nothing in its own file changed.

**A call that needs a human is refused, not held.** An approval card needs somebody to click it, and
nobody asked for this turn — there is no thread to put a card in and no one waiting on it. So the
...
turning this on: a rule that fires every morning is a task every morning.
