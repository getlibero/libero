...
| Enforcement, the vault, MCP client and pool, built-ins, listing bounds, budgets, why the budget read is advisory rather than a second enforcement point, approvals, the audit log's write discipline, what the hash chain catches and the four things it does not, why the unique index on `prev_hash` does more than the chain alone, why argument capture in the chain was declined rather than deferred, the off-chain attempt store built beside that decision (#364), and what the price-drift record holds, what it deliberately does not, and the three things that keep it from ever reaching a decision (#239) | `packages/proxy/README.md` |
