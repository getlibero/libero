...
Since #480 `sandbox` runs more than one, and since #484 it runs three:
`@getlibero/runner`, `@getlibero/litellm-conformance` and
`@getlibero/aws-conformance`, in series as three steps. Both of the runner's leak
assertions are filtered by image and by name, and neither a LiteLLM nor a
LocalStack container matches either, so there is no collision to buy more
runners out of. A new daemon-gated package has to make the same call, and
`ci-partition.test.ts` is what asks it the question — it names the gated
packages explicitly, so a fourth cannot be added without someone deciding where
it runs.
...
`DueEntry.kind`. Every rule is an ask; the deterministic post kind was declined
rather than deferred. `[ambient] heartbeat = false` runs rules and no evaluation,
where `enabled = false` stays the one silence. Occurrences are computed from the
wall clock with no last-fired stamp, so a restart cannot double-fire and a missed
window is skipped rather than replayed — the cost being that a restart spanning
Monday 09:00 loses that digest.

**One clause of its definition of done landed differently, and it is the one to
know.** "A fired turn induces no served calls" is still true of every sheet that
has not said otherwise, and is **no longer unconditional**: #348 was decided by
being built, so `[ambient] tools = true` gives a heartbeat, a fired check and a
standing rule the ReAct loop over the allowlist that sheet already carries. The
switch is off by default and grants nothing new — it decides *who may use* the
list, not what is on it. An unattended turn is handed no prompter, so a held call
is refused rather than waited on, which draws a read-yes-write-no line off the
destructive-name default; and every such call carries `AMBIENT_REQUESTING_USER`,
reserved by an alphabet no user id can spell, so the audit log says plainly that a
clock asked. What survives unchanged is the narrower claim: **injection cannot
plant a rule**, because the sheet is the only write path and the model has none.

Three things shipped that the milestone's own wording did not name, and the
release was better for not deferring them: an IANA `timezone` on a rule (#470,
absent meaning UTC so nothing earlier changed meaning, with the DST gap and the
repeated hour decided rather than left to the arithmetic); the heartbeat joining
the two fired turns behind the same switch (#471, the frequency objection having
...
| `packages/test-kit` | What `node:test` does not have and the suite needs: `it.each`, a `waitFor` whose timeout is a required argument, and the reporter — which fails a run that collected nothing, or that skipped something `ALLOWED_SKIPS` does not account for. Plus the three checks on the repository itself: every `test` script is one string, every package is run by exactly one CI job, and every hard-coded figure in the tree has a row on the limits page. Private, never published, no dependencies at all — which is what lets `packages/memory` import it across the leaf rule (#202) |
| `packages/aws-conformance` | One file asking whether the AWS custody backend's idea of Secrets Manager matches somebody else's (#484) — the contract suite from `packages/proxy/conformance` run against a real LocalStack. It found two defects the repository's own fake had mirrored, which is the argument for it. Private, never published, daemon-gated two-sided, run by the `sandbox` job |
...
| `packages/proxy` | The security boundary — mTLS listener, per-channel identity, team-sheet enforcement on both gates, the credential vault, the OAuth token store and the DPoP signing key on one custody seam — encrypted files by default, GCP Secret Manager and AWS Secrets Manager as managed backends — the token mint/refresh engine, injection and redaction, the MCP client over the official SDK and its pool, `search_channel_history` and `schedule_task` as built-ins, the budget meter in calls and in dollars, the append-only and hash-chained audit log, and the approval ticket store |
...
| `packages/memory` | The per-channel store — one SQLite file per channel, an FTS5 index, the delete and edit paths, the curated `MEMORY.md`, thread summaries and the two quiet-thread reads, a sqlite-vec embeddings table, the `skills/` directory and the index that follows it, the `proposals/` directory beside it, a read-only opener over the operator's shared skill root, `origin` on the skill index, the agent's own replies in a table of their own and the one read that sees them, and a read-only opener the proxy uses |
...
| Enforcement, the vault, the custody contract and what a backend inherits, MCP client and pool, built-ins, why `search_channel_history` leaves the calling thread out and what makes an asserted thread sound where an asserted channel would not be, listing bounds, budgets, why the budget read is advisory rather than a second enforcement point, approvals, the audit log's write discipline, what the hash chain catches and the four things it does not, why the unique index on `prev_hash` does more than the chain alone, why argument capture in the chain was declined rather than deferred, the off-chain attempt store built beside that decision (#364), what the price-drift record holds, what it deliberately does not, and the three things that keep it from ever reaching a decision (#239), where the custody contract is declared, what a backend may not relax about time, and why the failure vocabulary has two levels (#482), what the Secret Manager backend makes IAM's job rather than an import list's, the two costs of one secret per credential name, why no client library, and what its suite does not prove (#483), why the signing secret alone is written by stacking and read by version number and what licenses that (#529), and why the AWS one carries SigV4, what it gives up of AWS's versioning convention on purpose, and why removal there is irreversible (#484) | `packages/proxy/README.md` |
| Sessions and the queue, follow-ups, the transcript a task starts from, the checklist, the approvals client half, the environment contract, where recall and skill retrieval enter a task and why neither is a tool, why the transcript stopped being one-sided without becoming a dialogue and what the per-reply cap answers, why one embedding serves both, how the two skill legs are fused and what bounds them, why the post-reply turns are one thunk, what counts toward the author threshold, what bounds the quiescence sweep, why skills are embedded on channel activity rather than at task head, what `stale` means to retrieval and why, how the lifecycle job tells a hand-set status from its own, why a merge proposal is a file rather than a message, why the ambient clock enumerates the filesystem, wakes at the next due instant, and skips the windows it was down for, why the proactive post surface is minted in the composition, why its window is four hours, why its two sources are named for the wake reason, and what the heartbeat's pregate asks in what order, why its watermark makes a finding say-once, why a shut window defers rather than loses, which five turns compose the standing region and the composition-against-record line that decides, why membership is filtered before the fusion and what that still cannot un-spend, and what the skill-index purge deletes and the three things it does not, why a rule's next occurrence is computed rather than stamped and what that costs on a restart, which two days a year the clock has to decide about and how, why a rule's schedule is keyed by name, why one turn serves a check and a rule and where the two ends differ, and what `[ambient] tools` selects and the two questions it answers | `apps/server/README.md` |
| Slack normalization, the three subscriptions, card rendering, how the app learns its own id and its workspace from one `auth.test`, the three conditions under which one bot message survives the `bot_id` drop and what a consumer owes it, why the channel-post verb is a second exception to the `CardPoster` narrowing and a different kind of one, the three rules that package keeps | `packages/gateway/README.md` |
| The four reads, the isolation boundary, the tokenizer, why `search` takes text, why it ANDs before it ORs and what `excludeThread` is applied inside rather than over, why the agent's own replies are a second table rather than a `kind` column and what that keeps them out of, why `MEMORY.md` has no lock, what `allowExtension` does and does not open, why the vec table is created lazily, why a thread summary has a shape, why reconciliation is the skill index's only writer, why `nearest` takes a kind, why `searchSkills` ORs its terms where `search` ANDs them, why `idleThreads` is not `staleThreads` with another argument, why the lifecycle job's two stamps are two methods rather than one, why the proposals directory has no `read`, and why no trigger drops a considered pair | `packages/memory/README.md` |
| Operator commands and the vault CLI, what `audit verify`'s four exit codes are a contract for, and why the `drift` command has no failing exit code at all | `apps/proxy-server/README.md` |
| The shapes both services agree on. No README — each file's own header is the record, `src/skill.ts` and `src/audit.ts` most of all, and `src/names.ts` for why a clock's calls carry a sentinel no user id can spell | `packages/schema/src/*.ts` |
...
| Images, mounts, `.dockerignore` as an allowlist, the sandbox runner's service, networks and the one variable an operator gets wrong, the two managed-secret shapes and the IAM policies an operator applies, and the three ways to reach a model — why direct, a LiteLLM the operator already runs, and the sidecar this file starts are three chosen shapes rather than a default and two fallbacks, which of them a claim about the alias or the token envelope covers, what the external-gateway shape does not read and does not get, why the provider keys are prefixed and the agent holds none, why the master key is wired to the agent's own required variable rather than a compose guard, and why a `model_name` alias is what the price table must be keyed by | `deploy/README.md` |
...
| Why the custody contract is worth running against somebody else's implementation of an API, the two defects that found, what LocalStack cannot check and where the signature is checked instead | `packages/aws-conformance/README.md` |
| Why the test helpers are a package rather than a copied file, what is deliberately not re-exported from it, why `waitFor`'s timeout has no default, what the reporter fails a run for, the two things `node:test` does that it has to work around, and why the three checks that are not helpers live there | `packages/test-kit/README.md` |
| Vendored third-party source: a copy, not a fork | `packages/proxy/src/vendor/README.md` |
| Which figure bounds what, which process reads it, and whether it is fixed, a sheet field or a deployment's to set — the whole list, and the four figures that are written twice and kept in step by hand | `site/src/content/docs/docs/limits.md` |
