...
and a `tsconfig.json` extending `../../tsconfig.base.json`. A package with no
...
That is why the script is one string rather than a convention, and why
`packages/test-kit/src/test-scripts.test.ts` checks every package against it.
...
| `packages/test-kit` | The two things `node:test` does not have and the suite needs: `it.each` and a `waitFor` whose timeout is a required argument. Private, never published, no dependencies at all — which is what lets `packages/memory` import it across the leaf rule (#202) |
...
| Why the two test helpers are a package rather than a copied file, what is deliberately not re-exported from it, and why `waitFor`'s timeout has no default | `packages/test-kit/README.md` |
