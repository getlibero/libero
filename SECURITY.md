...
6. **Sandboxed code execution.** `run_code` runs model-written code in an ephemeral container the proxy invokes — read-only rootfs, tmpfs workdir, cpu/memory/wall-time caps, and no network unless the team sheet grants an egress allowlist. It is granted by a `[[builtin]]` block, approval-required by default, metered and audited like any other tool. `[egress]` is enforced rather than only validated (#219): the container's only route out is a filter that checks the channel's list per host, and a host outside it ends the run and refuses the call, naming the host on the audit row. The runner that starts containers holds the Docker socket and no credential; the proxy holds every credential and never the socket (#393).
