...
inlines into the bundle. It is read off **every inlined workspace package's** dependencies rather
than off one hardcoded path — it named `../schema/` while the schema was the only one, and #272
added `@getlibero/atomic-write` beside it. That second package declares no dependencies, so the
output did not change, which is exactly why it was worth fixing: the generator's claim was about to
become true only by coincidence, and the next package inlined here would have gone unnoticed. A
bundled dependency with no discoverable licence file fails the build rather than shipping
unattributed — `scripts/license-check.sh` asks which licences are *allowed*, and this is the other
half.

**A package this bundle inlines must declare nothing the bundle cannot carry.** An edge to
`@getlibero/memory` would drag `sqlite-vec` into the one artifact people install from npm, which is
why the durable-replace recipe got a package of its own rather than a home in the message store.
