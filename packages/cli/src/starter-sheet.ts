...
export function renderStarterSheet(sheet: StarterSheet): string {
  ...
  return [
    ...
    "#",
    "# Memory and skills are on: the agent curates a MEMORY.md after each reply,",
    "# summarizes quiet threads, and keeps a small library of skills it authors",
    "# out of its own tool-heavy tasks — all capped, in its own state root,",
    "# reaching nothing outside this channel. Opt out per block: [memory] or",
    "# [skills] with enabled = false. Ambient stays off until asked for.",
    "",
    "[channel]",
